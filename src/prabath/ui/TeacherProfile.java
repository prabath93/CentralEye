...
public class TeacherProfile extends javax.swing.JDialog {
    ...
    public TeacherProfile(java.awt.Frame parent, boolean modal, TeacherDAO teacherDAO, Teacher selectedTeacher) {
        ...
        System.out.println("insidd");
        System.out.println("nic:" + selectedTeacher.getNIC());
        populateGUI(selectedTeacher);
        oldNameWithIn = selectedTeacher.getNameWithin();
        oldFullname = selectedTeacher.getFullName();
        oldNIC = selectedTeacher.getNIC();
    ...
    public void populateGUI(Teacher teacher) {
        ...
        if (teacher.getDateOfAssignAsTeacher() != null) {
            doaDateChooser.setDate(new java.util.Date(teacher.getDateOfAssignAsTeacher().getTime()));
        }
        if (teacher.getDateOfAssignToSchool() != null) {
            dosDateChooser.setDate(new java.util.Date(teacher.getDateOfAssignToSchool().getTime()));
        }
        if (teacher.getDateOfPromotion() != null) {
            doaDateChooser.setDate(new java.util.Date(teacher.getDateOfPromotion().getTime()));
        }
        ...
        if (teacher.getServiceRecord() != null) {
            manageTables1.loadToTable(teacher.getServiceRecord());
        }
    ...
    private void initComponents() {
        ...
        titleLable.setFont(new java.awt.Font("Times New Roman", 3, 18)); // NOI18N
        titleLable.setText("My Profile");
        ...
        wrning3.setText("you cant change the NIC for that you have to meet principal or clerk");
    ...
    private void saveBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveBtnActionPerformed
        ...
        if (k == 0) {
            ...
            try {
                ...
                if (oldNIC.equals(nic) && oldFullname.equals(fullName) && oldNameWithIn.equals(nameWithIn)) {
                    System.out.println("fine");
                    teacherDAO.updateTeacher(tempTeacher, previousNIC);
                    JOptionPane.showMessageDialog(null, "Changes are saved successfully");
                } else {
                    if (selectedTeacher.getAccessPriviledge() == 1) {
                        teacherDAO.updateTeacher(tempTeacher, previousNIC);
                        JOptionPane.showMessageDialog(null, "Changes are saved successfully");
                    } else {
                        Notification notification = new Notification("Teacher", selectedTeacher.getNameWithin(), "Teacher", selectedTeacher.getNameWithin(), tempTeacher);
                        notificationDao.addNotification(notification);
                        JOptionPane.showMessageDialog(this, "Sent for confirmation");
                    }
    ...
    public void changeItems() {
        ...
        this.wrning1.setEnabled(false);
        this.wrning2.setEnabled(false);
        this.wrning3.setEnabled(false);
        this.titleLable.setText("Teacher Details");
        this.add1Btn.setText("See");
        this.jButton1.setText("See");
